...
interface ApiReply {
  ...
  isLiked: boolean;
  isReposted: boolean;
  nestedReplies?: ApiReply[];
}

// Helper function to recursively convert API reply to NestedReply type
function convertReply(reply: ApiReply): NestedReply {
  return {
    id: reply.id,
...
// Type for replies response
interface RepliesResponse {
  replies: ApiReply[];
  nextCursor?: string;
}

// Type for initial SSR data
export interface PostDetailInitialData {
  post: {
    id: string;
    content: string;
    authorId: string;
...
interface PostDetailClientProps {
  postId: string;
  initialData?: PostDetailInitialData;
}

export function PostDetailClient({ postId, initialData }: PostDetailClientProps) {
  ...
  // Use authenticated endpoints if logged in, otherwise use public endpoints with initial SSR data
  const isAuthenticated = !!session;

  // Authenticated queries
  const authPost = usePost(postId);
  const authThread = usePostThread(postId);
  const authReplies = usePostReplies(postId);

  // Public queries (with SSR data)
  const publicPost = usePublicPost(postId, initialData?.post);
  const publicThread = usePublicPostThread(postId, initialData?.thread ? { thread: initialData.thread } : undefined);
  const publicReplies = usePublicPostReplies(postId, 20, initialData?.replies);
  ...
  const {
    ...
    error: postError,
    refetch: refetchPost,
  } = isAuthenticated ? authPost : publicPost;

  const {
    data: threadData,
    isLoading: isLoadingThread,
    refetch: refetchThread,
  } = isAuthenticated ? authThread : publicThread;
  ...
  } = isAuthenticated ? authReplies : publicReplies;

  // Mutations (only work when authenticated)
  const createPostMutation = useCreatePost();
  const likePostMutation = useLikePost();
  const repostPostMutation = useRepostPost();

  // Flatten replies pages
  const replies = useMemo((): ApiReply[] => {
    if (!repliesData?.pages) return [];
    return repliesData.pages.flatMap((page) => page.replies as ApiReply[]);
  }, [repliesData]);

  // Get thread (parent posts)
  const thread = useMemo((): ThreadPost[] => {
    return (threadData?.thread ?? []) as ThreadPost[];
  }, [threadData]);
  ...
  const findPostInReplies = (targetId: string, repliesList: ApiReply[]): ApiReply | null => {
    for (const reply of repliesList) {
      if (reply.id === targetId) return reply;
      if (reply.nestedReplies?.length && reply.nestedReplies.length > 0) {
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex flex-col items-center justify-center p-12 text-center">
          ...
          <p className="text-muted-foreground mb-4">
            {(postError as Error | null)?.message || "Post not found"}
          </p>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {isLoadingThread && !initialData?.thread ? (
      ...
      {/* Replies */}
      {isLoadingReplies && !initialData?.replies ? (
        <div className="divide-y">
          {[...Array(3)].map((_, i) => (
